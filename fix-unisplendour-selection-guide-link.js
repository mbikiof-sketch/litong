...
console.log('修复 selectionGuideLink 和其他剩余问题...\n');

// 1. 修复products.json中的selectionGuideLink (需要是对象格式 {url, text})
products.categories.forEach(cat => {
  if (typeof cat.selectionGuideLink === 'string') {
    ...
    console.log(`✅ 修复分类 ${cat.name} 的 selectionGuideLink 格式`);
  } else if (!cat.selectionGuideLink) {
    cat.selectionGuideLink = {
      "url": `/brands/unisplendour/support/${cat.id}-selection-guide/`,
      "text": `View ${cat.name} Selection Guide`
    };
    console.log(`✅ 创建分类 ${cat.name} 的 selectionGuideLink`);
  }
});
...
// 2. 修复solutions.json中的customerCases (需要challenge/solution/result字段)
...
// 3. 修复support.json中的faeInsights长度 (4篇文章)
