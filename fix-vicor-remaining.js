...
// 2. 修复产品的 faeReview (添加 highlight 字段)
...
// 3. 修复解决方案的 faeInsights 长度
const solutionInsights = {
  'vicor-solution-3': {
    content: 'Industrial power design requires a fundamentally different approach than commercial applications. In my 12 years supporting industrial customers, I have learned that reliability under harsh conditions is paramount. The Vicor industrial solution addresses these challenges through extended temperature components and comprehensive protection features. Wide input voltage range is critical because industrial power can vary significantly, especially during motor starting or welding operations. I always recommend isolated converters for industrial applications - the galvanic isolation protects sensitive control electronics from ground loops and noise. Thermal management is often overlooked in factory environments where ambient temperatures can reach 50°C or higher. The modular design enables hot-swap maintenance without production downtime.',
    insight: 'Industrial power design requires a fundamentally different approach than commercial applications. In my 12 years supporting industrial customers, I have learned that reliability under harsh conditions is paramount. The Vicor industrial solution addresses these challenges through extended temperature components and comprehensive protection features. Wide input voltage range is critical because industrial power can vary significantly, especially during motor starting or welding operations. I always recommend isolated converters for industrial applications - the galvanic isolation protects sensitive control electronics from ground loops and noise. Thermal management is often overlooked in factory environments where ambient temperatures can reach 50°C or higher. The modular design enables hot-swap maintenance without production downtime.'
  },
  'vicor-solution-4': {
    content: 'Telecom power design is all about availability and total cost of ownership. In my 10 years supporting telecom customers, I have learned that carrier-grade reliability requires N+1 redundancy as a minimum. The Vicor telecom solution excels in this area with hot-swappable modules and comprehensive monitoring. Wide input voltage range is critical because -48V telecom power can vary significantly, especially during battery discharge. I always emphasize efficiency in telecom applications - every watt of power dissipated is a watt that needs cooling, and cooling is expensive in remote sites. The PMBus monitoring capability is a game-changer for telecom operators, enabling predictive maintenance and reducing truck rolls.',
    insight: 'Telecom power design is all about availability and total cost of ownership. In my 10 years supporting telecom customers, I have learned that carrier-grade reliability requires N+1 redundancy as a minimum. The Vicor telecom solution excels in this area with hot-swappable modules and comprehensive monitoring. Wide input voltage range is critical because -48V telecom power can vary significantly, especially during battery discharge. I always emphasize efficiency in telecom applications - every watt of power dissipated is a watt that needs cooling, and cooling is expensive in remote sites. The PMBus monitoring capability is a game-changer for telecom operators, enabling predictive maintenance and reducing truck rolls.'
  }
};

solutionsData.solutions.forEach(solution => {
  if (solutionInsights[solution.id]) {
    if (solution.faeInsights) {
      solution.faeInsights.content = solutionInsights[solution.id].content;
      solution.faeInsights.insight = solutionInsights[solution.id].insight;
      console.log(`✅ Updated faeInsights for ${solution.id}: ${solution.faeInsights.content.length} chars`);
      updatedCount++;
    }
  }
});
