  ...
  return [
    {
      ...
      specifications: { 
        ...specs,
        "Efficiency": specs.Efficiency ? specs.Efficiency.replace(/\d+%/, match => `${parseInt(match) + 2}%`) : "Up to 97%",
        "Quiescent Current": specs["Quiescent Current"] ? "Lower than standard" : "Ultra-low"
      },
      comparison: {
        "Efficiency": "Higher than standard version (+2%)",
...
console.log('\n✅ Products data fixed and saved!');
console.log('\nSummary:');
productsData.categories.forEach(cat => {
  console.log(`  ${cat.name}: ${cat.products.length} products, each with 8 FAQs`);
});
