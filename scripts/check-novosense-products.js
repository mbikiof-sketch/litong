#!/usr/bin/env node
/**
 * Check Novosense product count per category
 */
...
productsData.categories.forEach(cat => {
  ...
  if (cat.products && cat.products.length > 0) {
    ...
    cat.products.forEach((prod, idx) => {
      ...
      console.log(`     ${idx + 1}. ${prod.partNumber} - FAQs:${faqCount} FAE:${hasFaeReview} Alt:${hasAltParts}`);
    });
  }
  console.log('');
});
