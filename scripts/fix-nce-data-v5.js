#!/usr/bin/env node
/**
 * NCE Brand Data Fix Script V5
 * Fixes selectionGuideLink format and customer cases
 */
...
// Fix products.json - Fix selectionGuideLink format to be an object
function fixProducts() {
  console.log('\n=== Fixing products.json ===');
  const data = readJSON('products.json');

  data.categories.forEach(category => {
    // Fix selectionGuideLink - should be an object with url and text
    if (typeof category.selectionGuideLink === 'string' || !category.selectionGuideLink) {
      category.selectionGuideLink = {
        url: `/nce/support/mosfet-selection-guide.html`,
...
// Main execution
console.log('Starting NCE brand data fix V5...');
