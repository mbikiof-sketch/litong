/**
 * 修复BYD品牌中编造的产品型号
 * 用真实产品型号替换BG500F12A13L5和BG600F12A13L5
 */
...
if (igbtCategory && igbtCategory.products) {
  ...
  // 替换编造的产品
  let replacedCount = 0;
  fabricatedParts.forEach((fabricatedPart, index) => {
    const productIndex = igbtCategory.products.findIndex(p => p.partNumber === fabricatedPart);
    if (productIndex !== -1 && index < realProducts.length) {
      const realProduct = createCompleteProduct(realProducts[index]);
      igbtCategory.products[productIndex] = realProduct;
  ...
  console.error('未找到IGBT模块分类');
}

function createCompleteProduct(template) {
  return {
