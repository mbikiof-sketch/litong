...
console.log('=== 3peak Brand Data Check ===\n');
...
data.categories.forEach((cat, idx) => {
  ...
  if (cat.products) {
    cat.products.forEach((prod, pidx) => {
      ...
      // Check for placeholder alternative parts
      if (prod.alternativeParts?.some(alt => alt.partNumber?.includes('ALT'))) {
        issues.push('PLACEHOLDER_ALTERNATIVE');
      }
