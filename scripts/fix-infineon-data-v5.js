#!/usr/bin/env node
/**
 * Infineon Brand Data Fix Script V5
 * Fixes product FAQ answer lengths and alternativeParts
 */
...
// Fix products.json - Fix product FAQ answer lengths and alternativeParts
  ...
  data.categories.forEach(category => {
    ...
    if (category.products) {
      category.products.forEach(product => {
        // Fix product FAQs - ensure answers are at least 200 characters
        if (product.faqs) {
          product.faqs.forEach(faq => {
            if (faq.answer && faq.answer.length < 200) {
              faq.answer += ` For more detailed information about ${product.partNumber} and application guidance, please consult the product datasheet or contact our technical support team. Our FAE engineers are available to assist with your specific design requirements and provide personalized recommendations for your project.`;
            }
          });
        }
...
// Main execution
console.log('Starting Infineon brand data fix V5...');

try {
  fixProducts();
  console.log('\n✓ All V5 fixes completed successfully!');
